...
// Hover animation for overlays
            document.querySelectorAll("div[style*='position:relative']").forEach(card => {
                let overlay = card.querySelector("div[style*='position:absolute']");
                card.addEventListener("mouseenter", () => { overlay.style.opacity = "1"; });
                card.addEventListener("mouseleave", () => { overlay.style.opacity = "0"; });
            });

        // Open overlay
        document.querySelector(".header_btn").addEventListener("click", function(e){
            e.preventDefault();
